...
bc <- get_eztune_results("../BreastCancer2", "Breast Cancer", "ada")
io <- get_eztune_results("../Ionosphere2", "Ionosphere", "ada")
li <- get_eztune_results("../Lichen2", "Lichen", "ada")
mu <- get_eztune_results("../Mullein2", "Mullein", "ada")
pi <- get_eztune_results("../Pima2", "Pima", "ada")
so <- get_eztune_results("../Sonar2", "Sonar", "ada")
...
g1 <- ggplot(std_dat, aes(x = type, y = perf_std, group = factor(Dataset))) +
  ...
  labs(title ="Adaboost", x = "", y = "Standardized Error Rate") +
...
pdf("../Images/ada_bin_eztune.pdf", width = 6, height = 7.5)
...
ada_bin_results <- results_table(dat = std_dat, best = best_err, 
                                 datasets = data_sets, model = "ADA")

write.table(ada_bin_results[[2]], "../Tables/ada_bin_results.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
