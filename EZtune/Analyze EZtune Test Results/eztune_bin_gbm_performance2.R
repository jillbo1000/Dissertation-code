...
bc <- get_eztune_results("../BreastCancer2", "Breast Cancer", "gbm")
io <- get_eztune_results("../Ionosphere2", "Ionosphere", "gbm")
li <- get_eztune_results("../Lichen2", "Lichen", "gbm")
mu <- get_eztune_results("../Mullein2", "Mullein", "gbm")
pi <- get_eztune_results("../Pima2", "Pima", "gbm")
so <- get_eztune_results("../Sonar2", "Sonar", "gbm")
...
g1 <- ggplot(std_dat, aes(x = type, y = perf_std, group = factor(Dataset))) +
  ...
  labs(title ="Gradient Boosting Machine", x = "", y = "Standardized Error Rate") +
...
pdf("../Images/gbm_bin_eztune.pdf", width = 6, height = 7.5)
...
gbm_bin_results <- results_table(dat = std_dat, best = best_err, 
                                 datasets = data_sets, model = "GBM")

write.table(gbm_bin_results[[2]], "../Tables/gbm_bin_results.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
