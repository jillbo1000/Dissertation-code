...
ab <- get_eztune_results("../Abalone2", "Abalone", "gbm")
bh <- get_eztune_results("../BostonHousing2", "Boston Housing", "gbm")
co2 <- get_eztune_results("../CO22", "CO2", "gbm")
cr <- get_eztune_results("../Crime2", "Crime", "gbm")
oh <- get_eztune_results("../OhioHousing2", "Ohio Housing", "gbm")
un <- get_eztune_results("../Union2", "Union", "gbm")
wa <- get_eztune_results("../Wage2", "Wage", "gbm")
...
g1 <- ggplot(std_dat, aes(x = type, y = perf_std, group = factor(Dataset))) +
  ...
  labs(title ="Gradient Boosting Regression", x = "", y = "Standardized MSE") +
...
pdf("../Images/gbm_reg_eztune.pdf", width = 6, height = 7.5)
...
gbm_reg_results <- results_table(dat = std_dat, best = best_mse, 
                                 datasets = data_sets, model = "GBM")

write.table(gbm_reg_results[[2]], "../Tables/gbm_reg_results.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
