...
results_table <- function(dat, best, datasets, model) {
  ...
  tab <- matrix(NA, nrow = length(unique(dat$type)), ncol = length(datasets) + 2)
  ...
  for(i in 1:nrow(tab)) {
    for(j in 3:ncol(tab)) {
      err <- NA
      try(err <- trim(dat$mean_perf_cv[(dat$optimizer == tab[i, 1] & as.character(dat$type) == tab[i, 2] & dat$Dataset == colnames(tab)[j])]))
      try(tim <- round(dat$mean_time[(dat$optimizer == tab[i, 1] & as.character(dat$type) == tab[i, 2] & dat$Dataset == colnames(tab)[j])], 0))
      if(!is.na(err)) {
  ...
  for(i in 1:nrow(best)) {
    bg[i] <- trim(best[best$Data == datasets[i], colnames(best) == model])
  }
  bg <- c("Best Grid", "", bg)
  tab <- rbind(tab, bg)
