  ...
  for(l in 1:length(win)) {
    for(m in 1:length(filtnum)) {
      for(n in 1:length(datnum)) {
        ...
        plot_file <- paste(set, "_h0", h[i], "_", size[j], "_", filt[k], "_w", win[l], "_", 
                           datnum[n], "_", filtnum[m], ".png", sep = "")
        
        if(!file.exists(paste(deffects, "plots", sep = ""))){
          dir.create(file.path(deffects, "plots"))
        } 
        
        ttl <- paste("R. pomonella, Heritability = 0.", h[i], ", ", size[j], 
                     " Effective SNPs, ", "\n Filter = ", filt[k], ", ",
                     "Window = ", win[l], ", ", filtnum2[m], ", Data = ", datnum2[n], sep = "")
        png(paste(deffects, "plots/", plot_file, sep = ""), height = 500, width = 600)
        par(mfrow=c(3,3), oma = c(0, 0, 4, 0), mar = c(2, 2, 2.5, 2))
