...
test <- switch(data_name, 
               "Lichen" = lichenTest, 
               "Mullein" = mulleinTest)

colnames(test)[colnames(test) == colnames(dat)[1]] = "y"
colnames(dat)[1] = "y"
...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  try(pr <- ada(as.factor(y) ~ ., loss = "exponential", iter = param[i, 2],
                nu = param[i, 1], data = dat, control = rpart.control(maxdepth = param[i, 3])))

  if(!is.null(pr)) {
    param[i, 4] <- mean(round(predict(pr, newdata = test, type = "prob")[,2]) == test$y)
    param[i, 5] <- mean(round(predict(pr, newdata = test, type = "prob")[,2]) == test$y) 
  }
...
ff <- paste(f3, data_name, "Test/", data_name, f4, file_num, ".csv", sep = "")
