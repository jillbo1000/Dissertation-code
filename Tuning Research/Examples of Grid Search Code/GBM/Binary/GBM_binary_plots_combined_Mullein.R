# set working directory to muurce file
...
mug <- datagridClassGBM2("C:/Users/jflun/Dropbox/Dissertation/Tuning Research/Grid Search/Grid Data/GBM/Binary/Mullein Small",
                         "C:/Users/jflun/Dropbox/Dissertation/Tuning Research/Grid Search/Grid Data/GBM/Binary/Mullein",
                         dataset = "Mullein")
...
dat1A <- mug$dat20A
dat1LCL <- mug$dat20LCL
dat1Time <- mug$dat20Time
...
tmp3 <- ggplot(mu1.1, aes(x = IntDepth, y = NumTrees)) +
  geom_tile(aes(fill = Error), fill ="white") +
  theme_bw() +
  labs(x = "interaction.depth", y = "Number of Trees") +
  ggtitle("Best 20% Errors for Mullein Data, Shrinkage = 10^(-2)") +
...
tmp4 <- ggplot(mu1.1, aes(x = IntDepth, y = NumTrees)) +
  geom_tile(aes(fill = Error), fill ="white") +
  theme_bw() +
  labs(x = "interaction.depth", y = "Number of Trees") +
  ggtitle("Best 20% UCLs for Mullein Data, Shrinkage = 10^(-2)") +
...
pdf("../Grid Search Plots/GBM/Binary/GBM_Binary_Complete_Mullein.pdf", height = 9, width = 6.5)
grid.arrange(g1.1, g1.2, g1.3, ncol = 1)
# g4.3 does not have any data
grid.arrange(g4.1, tmp3, tmp1, ncol = 1)
grid.arrange(g2.1, g2.2, g2.3, ncol = 1)
# g5.3 does not have any data
grid.arrange(g5.1, tmp4, tmp2, ncol = 1)
