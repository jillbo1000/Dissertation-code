# set working directory to piurce file

library(tidyverse)
library(gridExtra)

source("C:/Users/jflun/Dropbox/Dissertatpin/Tuning Research/Grid Search/Plot Scripts/get_data.R")


pig <- datagridClassGBM2("C:/Users/jflun/Dropbox/Dissertation/Tuning Research/Grid Search/Grid Data/GBM/Binary/Pima Small",
                         "C:/Users/jflun/Dropbox/Dissertation/Tuning Research/Grid Search/Grid Data/GBM/Binary/Pima",
                         dataset = "Pima")
...
dat1A <- pig$dat20A
dat1LCL <- pig$dat20LCL
dat1Time <- pig$dat20Time
...
tmp1 <- ggplot(pi1.1, aes(x = IntDepth, y = NumTrees)) +
...
tmp2 <- ggplot(pi1.1, aes(x = IntDepth, y = NumTrees)) +
  geom_tile(aes(fill = Error), fill ="white") +
  theme_bw() +
  labs(x = "interaction.depth", y = "Number of Trees") +
  ggtitle("Best 20% UCLs for Pima Data, Shrinkage = 10^(-3)") +
...
tmp3 <- ggplot(pi1.1, aes(x = IntDepth, y = NumTrees)) +
  geom_tile(aes(fill = Error), fill ="white") +
  theme_bw() +
  labs(x = "interaction.depth", y = "Number of Trees") +
  ggtitle("Best 20% Errors for Pima Data, Shrinkage = 10^(-2)") +
...
tmp4 <- ggplot(pi1.1, aes(x = IntDepth, y = NumTrees)) +
  geom_tile(aes(fill = Error), fill ="white") +
  theme_bw() +
  labs(x = "interaction.depth", y = "Number of Trees") +
  ggtitle("Best 20% UCLs for Pima Data, Shrinkage = 10^(-2)") +
...
pdf("../Grid Search Plots/GBM/Binary/GBM_Binary_Complete_Pima.pdf", height = 9, width = 6.5)
