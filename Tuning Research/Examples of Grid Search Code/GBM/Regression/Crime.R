...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  pr <- NULL
  try(pr <- cv.pred(crime[, -1], crime[, 1], fold = 10, tr = param[i, 1], 
...
f1 <- paste("/scratch/general/lustre/u6007925/Grid/GBM/Regression/Crime/Crime_Small_Grid_", file_num, ".csv", sep = "")
