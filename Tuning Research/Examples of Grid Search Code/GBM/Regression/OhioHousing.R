...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  pr <- NULL
  try(pr <- cv.pred(oh[, -1], oh[, 1], fold = 3, tr = param[i, 1], 
...
f1 <- paste("/scratch/general/lustre/u6007925/Grid/GBM/Regression/OhioHousing/Ohio_Housing_Small_Grid_", file_num, ".csv", sep = "")
