...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  try(mod <- svm(as.factor(LobaOreg) ~ ., data = lich, gamma = 2^param[i, 2], 
...
f1 <- paste("/scratch/general/lustre/u6007925/Grid/SVM/Binary/Lichen/Lichen_Grid_", file_num, ".csv", sep = "")
