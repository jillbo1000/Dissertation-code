...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  try(mod <- svm(as.factor(VerbThap) ~ ., data = mullein, gamma = 2^param[i, 2], 
                 cost = 2^param[i, 1], cross = 3))
...
f1 <- paste("/scratch/general/lustre/u6007925/Grid/SVM/Binary/Mullein/Mullein_Grid_", file_num, ".csv", sep = "")
