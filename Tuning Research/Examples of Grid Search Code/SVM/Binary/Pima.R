...
pid <- mutate(PimaIndiansDiabetes, lpregnant = log(pregnant + 1), 
              lpedigree = log(pedigree + 1), lage = log(age + 1)) %>%
  select(-pregnant, -pedigree, -age)
...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  try(mod <- svm(as.factor(diabetes) ~ ., data = pid, gamma = 2^param[i, 2], 
...
f1 <- paste("/scratch/general/lustre/u6007925/Grid/SVM/Binary/Pima/Pima_Grid_", file_num, ".csv", sep = "")
