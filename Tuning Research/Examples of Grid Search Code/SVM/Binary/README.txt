...
SVM_bin2.slurm: example bash file to do the runs on the CHPC. 

mySVMbin2.conf: command lines that were accessed by the slurm file. The numbers
...
SVM_binary_plots.R (* represents name of dataset): each of these grabs the grid 
data sets that were computed on the CHPC, combines them into a single data frame, 
creates a list of smaller data frames that make plotting easier, and then 
plots the grid data. T 
