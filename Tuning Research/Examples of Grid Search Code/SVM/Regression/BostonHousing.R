...
library("R.utils")
library(e1071)
library(mlbench) # BostonHousing2 and BreastCancer
library(tidyverse)
...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  pr <- NULL
  try(pr <- svm(cmedv ~ ., data = bh, gamma = 2^param[i, 2], 
...
f1 <- paste("/scratch/general/lustre/u6007925/Grid/SVM/Regression/BostonHousing/Boston_Housing_Grid_", file_num, ".csv", sep = "")
