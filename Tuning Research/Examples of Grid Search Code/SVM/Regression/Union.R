...
for(i in 1:nrow(param)) {
  t1 <- Sys.time()
  if(!is.null(tryCatch({
    svm(DENS ~ ., data = union, gamma = 2^param[i, 2], 
        cost = 2^param[i, 1], epsilon = param[i, 3], cross = 10)
  }, error=function(e){}))) {
    mod <- svm(DENS ~ ., data = union, gamma = 2^param[i, 2], 
               cost = 2^param[i, 1], epsilon = param[i, 3], cross = 10)
...
f1 <- paste("/scratch/general/lustre/u6007925/Grid/SVM/Regression/Union/Union_Grid_", file_num, ".csv", sep = "")
