...
for(i in 1:10) {
  t1 <- Sys.time()
  da.obj <- DA(FUN = ada.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(da.obj)) {
    alldat$Nu[i] <- da.obj[1]
    alldat$Iter[i] <- round(da.obj[2])
    alldat$Depth[i] <- round(da.obj[3])
    alldat$Error[i] <- ada.opt(da.obj)
  }
