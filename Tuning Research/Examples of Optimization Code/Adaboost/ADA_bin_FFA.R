...
for(i in 1:10) {
  t1 <- Sys.time()
  ffa.obj <- FFA(FUN = ada.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(ffa.obj)) {
    alldat$Nu[i] <- ffa.obj[1]
    alldat$Iter[i] <- round(ffa.obj[2])
    alldat$Depth[i] <- round(ffa.obj[3])
    alldat$Error[i] <- ada.opt(ffa.obj)
  }
