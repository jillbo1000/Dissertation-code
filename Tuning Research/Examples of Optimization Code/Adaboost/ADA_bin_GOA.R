...
for(i in 1:10) {
  t1 <- Sys.time()
  goa.obj <- GOA(FUN = ada.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(goa.obj)) {
    alldat$Nu[i] <- goa.obj[1]
    alldat$Iter[i] <- round(goa.obj[2])
    alldat$Depth[i] <- round(goa.obj[3])
    alldat$Error[i] <- ada.opt(goa.obj)
  }
