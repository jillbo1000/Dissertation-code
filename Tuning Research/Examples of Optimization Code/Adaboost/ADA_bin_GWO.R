...
for(i in 1:10) {
  t1 <- Sys.time()
  gwo.obj <- GWO(FUN = ada.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(gwo.obj)) {
    alldat$Nu[i] <- gwo.obj[1]
    alldat$Iter[i] <- round(gwo.obj[2])
    alldat$Depth[i] <- round(gwo.obj[3])
    alldat$Error[i] <- ada.opt(gwo.obj)
  }
