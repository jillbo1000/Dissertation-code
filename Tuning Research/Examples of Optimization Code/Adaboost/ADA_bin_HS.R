...
for(i in 1:10) {
  t1 <- Sys.time()
  hs.obj <- HS(FUN = ada.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(hs.obj)) {
    alldat$Nu[i] <- hs.obj[1]
    alldat$Iter[i] <- round(hs.obj[2])
    alldat$Depth[i] <- round(hs.obj[3])
    alldat$Error[i] <- ada.opt(hs.obj)
  }
