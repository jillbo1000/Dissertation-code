...
for(i in 1:10) {
  t1 <- Sys.time()
  mfo.obj <- MFO(FUN = ada.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(mfo.obj)) {
    alldat$Nu[i] <- mfo.obj[1]
    alldat$Iter[i] <- round(mfo.obj[2])
    alldat$Depth[i] <- round(mfo.obj[3])
    alldat$Error[i] <- ada.opt(mfo.obj)
  }
