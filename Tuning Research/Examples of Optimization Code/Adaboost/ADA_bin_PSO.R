...
for(i in 1:10) {
  t1 <- Sys.time()
  pso.obj <- PSO(FUN = ada.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(pso.obj)) {
    alldat$Nu[i] <- pso.obj[1]
    alldat$Iter[i] <- round(pso.obj[2])
    alldat$Depth[i] <- round(pso.obj[3])
    alldat$Error[i] <- ada.opt(pso.obj)
  }
