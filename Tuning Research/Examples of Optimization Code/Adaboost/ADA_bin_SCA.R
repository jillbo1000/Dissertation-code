...
for(i in 1:10) {
  t1 <- Sys.time()
  sca.obj <- SCA(FUN = ada.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(sca.obj)) {
    alldat$Nu[i] <- sca.obj[1]
    alldat$Iter[i] <- round(sca.obj[2])
    alldat$Depth[i] <- round(sca.obj[3])
    alldat$Error[i] <- ada.opt(sca.obj)
  }
