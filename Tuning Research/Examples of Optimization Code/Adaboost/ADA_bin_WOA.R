...
for(i in 1:10) {
  t1 <- Sys.time()
  woa.obj <- WOA(FUN = ada.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(woa.obj)) {
    alldat$Nu[i] <- woa.obj[1]
    alldat$Iter[i] <- round(woa.obj[2])
    alldat$Depth[i] <- round(woa.obj[3])
    alldat$Error[i] <- ada.opt(woa.obj)
  }
