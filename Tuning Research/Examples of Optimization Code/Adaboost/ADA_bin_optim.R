...
for(i in 1:10) {
  t1 <- Sys.time()
  optim.obj <- optim(par = c(nstart, istart, dstart), fn = ada.opt, 
                     lower = c(nmin, imin, dmin), upper = c(nmax, imax, dmax), 
                     method = "L-BFGS-B")
  t2 <- Sys.time()
  if(!is.null(optim.obj)) {
    alldat$Nu[i] <- optim.obj$par[1]
    alldat$Iter[i] <- round(optim.obj$par[2])
    alldat$Depth[i] <- round(optim.obj$par[3])
