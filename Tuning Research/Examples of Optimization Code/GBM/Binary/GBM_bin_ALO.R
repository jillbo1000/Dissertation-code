...
for(i in 1:10) {
  t1 <- Sys.time()
  alo.obj <- ALO(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(alo.obj)) {
    alldat$Trees[i] = alo.obj[1]
    alldat$Depth[i] = round(alo.obj[2])
    alldat$Nmin[i] = round(alo.obj[3])
    alldat$Shrinkage[i] = alo.obj[4]
    alldat$Error[i] = gbm.opt(alo.obj)
  }
