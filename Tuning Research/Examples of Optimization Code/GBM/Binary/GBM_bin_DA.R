...
for(i in 1:10) {
  t1 <- Sys.time()
  da.obj <- DA(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(da.obj)) {
    alldat$Trees[i] = da.obj[1]
    alldat$Depth[i] = round(da.obj[2])
    alldat$Nmin[i] = round(da.obj[3])
    alldat$Shrinkage[i] = da.obj[4]
    alldat$Error[i] = gbm.opt(da.obj)
  }
