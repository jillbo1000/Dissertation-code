...
for(i in 1:10) {
  t1 <- Sys.time()
  ffa.obj <- FFA(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(ffa.obj)) {
    alldat$Trees[i] = ffa.obj[1]
    alldat$Depth[i] = round(ffa.obj[2])
    alldat$Nmin[i] = round(ffa.obj[3])
    alldat$Shrinkage[i] = ffa.obj[4]
    alldat$Error[i] = gbm.opt(ffa.obj)
  }
