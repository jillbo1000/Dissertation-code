...
for(i in 1:10) {
  t1 <- Sys.time()
  goa.obj <- GOA(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(goa.obj)) {
    alldat$Trees[i] = goa.obj[1]
    alldat$Depth[i] = round(goa.obj[2])
    alldat$Nmin[i] = round(goa.obj[3])
    alldat$Shrinkage[i] = goa.obj[4]
    alldat$Error[i] = gbm.opt(goa.obj)
  }
