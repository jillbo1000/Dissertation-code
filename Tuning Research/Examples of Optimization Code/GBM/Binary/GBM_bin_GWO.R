...
for(i in 1:10) {
  t1 <- Sys.time()
  gwo.obj <- GWO(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(gwo.obj)) {
    alldat$Trees[i] = gwo.obj[1]
    alldat$Depth[i] = round(gwo.obj[2])
    alldat$Nmin[i] = round(gwo.obj[3])
    alldat$Shrinkage[i] = gwo.obj[4]
    alldat$Error[i] = gbm.opt(gwo.obj)
  }
