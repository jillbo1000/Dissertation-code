...
for(i in 1:10) {
  t1 <- Sys.time()
  hs.obj <- HS(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(hs.obj)) {
    alldat$Trees[i] <- hs.obj[1]
    alldat$Depth[i] <- round(hs.obj[2])
    alldat$Nmin[i] <- round(hs.obj[3])
    alldat$Shrinkage[i] <- hs.obj[4]
    alldat$Error[i] <- gbm.opt(hs.obj)
  }
