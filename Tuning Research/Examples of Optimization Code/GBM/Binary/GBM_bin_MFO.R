...
for(i in 1:10) {
  t1 <- Sys.time()
  mfo.obj <- MFO(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(mfo.obj)) {
    alldat$Trees[i] <- mfo.obj[1]
    alldat$Depth[i] <- round(mfo.obj[2])
    alldat$Nmin[i] <- round(mfo.obj[3])
    alldat$Shrinkage[i] <- mfo.obj[4]
    alldat$Error[i] <- gbm.opt(mfo.obj)
  }
