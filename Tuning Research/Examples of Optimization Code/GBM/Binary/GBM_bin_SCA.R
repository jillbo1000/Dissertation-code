...
for(i in 1:10) {
  t1 <- Sys.time()
  sca.obj <- SCA(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(sca.obj)) {
    alldat$Trees[i] <- sca.obj[1]
    alldat$Depth[i] <- round(sca.obj[2])
    alldat$Nmin[i] <- round(sca.obj[3])
    alldat$Shrinkage[i] <- sca.obj[4]
    alldat$Error[i] <- gbm.opt(sca.obj)
  }
