...
for(i in 1:10) {
  t1 <- Sys.time()
  woa.obj <- WOA(FUN = gbm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(woa.obj)) {
    alldat$Trees[i] <- woa.obj[1]
    alldat$Depth[i] <- round(woa.obj[2])
    alldat$Nmin[i] <- round(woa.obj[3])
    alldat$Shrinkage[i] <- woa.obj[4]
    alldat$Error[i] <- gbm.opt(woa.obj)
  }
