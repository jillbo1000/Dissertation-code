...
for(i in 1:10) {
  ...
  t2 <- Sys.time()
  if(!is.null(ga.obj)) {
    alldat$Trees[i] = round(ga.obj@solution[1, 1])
  ...
  print(ga.obj)
  print(alldat[i, ])
  print(paste(i, ", Trees = ", round(ga.obj@solution[1, 1]), ", Depth = ", 
              round(ga.obj@solution[1, 2]), ", Nmin = ", round(ga.obj@solution[1, 3]), 
              ", Shrinkage = ", ga.obj@solution[1, 4], ", Error = ", 1 - ga.obj@fitnessValue, 
              sep = ""))
