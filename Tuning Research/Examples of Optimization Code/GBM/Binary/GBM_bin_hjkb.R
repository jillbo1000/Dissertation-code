...
for(i in 1:10) {
  t1 <- Sys.time()
  hjkb.obj <- hjkb(par = c(tstart, dstart, nstart, sstart), fn = gbm.opt, 
                   lower = c(tmin, dmin, nmin, smin), 
                   upper = c(tmax, dmax, nmax, smax))
  t2 <- Sys.time()
  if(!is.null(hjkb.obj)) {
    alldat$Trees[i] = hjkb.obj$par[1]
    alldat$Depth[i] = round(hjkb.obj$par[2])
    alldat$Nmin[i] = round(hjkb.obj$par[3])
    alldat$Shrinkage[i] = hjkb.obj$par[4]
    alldat$Error[i] = hjkb.obj$value
  }
