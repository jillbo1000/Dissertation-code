...
GBM_bin_opt1.slurm: example bash file that controls the computations done 
on the CHPC. 

myGBMopt1.conf: file that contains the command lines called in 
GBM_bin_opt1.slurm. The numbers and strings at the end of the R file are 
arguments that are passed to the R script. 

GBM_bin_*.R (* represents the optimization algorithm): these did 10 runs for 
...
GBM_opt_funs.R: contains the functions that bring in the data and the functions
that the optimization functions optimize. 

GBM_bin_opt_graphs: reads in all of the datasets from the optimization tests
