...
for(i in 1:10) {
  t1 <- Sys.time()
  Rcgmin.obj <- Rcgmin(par = c(tstart, dstart, nstart, sstart), fn = gbm.opt, 
                       lower = c(tmin, dmin, nmin, smin), 
                       upper = c(tmax, dmax, nmax, smax))
  t2 <- Sys.time()
  if(!is.null(Rcgmin.obj)) {
    alldat$Trees[i] <- Rcgmin.obj$par[1]
    alldat$Depth[i] <- round(Rcgmin.obj$par[2])
    alldat$Nmin[i] <- round(Rcgmin.obj$par[3])
    alldat$Shrinkage[i] <- Rcgmin.obj$par[4]
    alldat$MSE[i] <- Rcgmin.obj$value
  }
