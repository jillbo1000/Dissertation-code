...
for(i in 1:10) {
  t1 <- Sys.time()
  spg.obj <- spg(par = c(tstart, dstart, nstart, sstart), fn = gbm.opt, 
                 lower = c(tmin, dmin, nmin, smin), 
                 upper = c(tmax, dmax, nmax, smax))
  t2 <- Sys.time()
  if(!is.null(spg.obj)) {
    alldat$Trees[i] <- spg.obj$par[1]
    alldat$Depth[i] <- round(spg.obj$par[2])
    alldat$Nmin[i] <- round(spg.obj$par[3])
    alldat$Shrinkage[i] <- spg.obj$par[4]
    alldat$MSE[i] <- spg.obj$value
  }
