...
GBM_reg_opt1.slurm: example bash file that controls the computations done 
on the CHPC. 

opt1.conf: file that contains the command lines called in 
GBM_reg_opt1.slurm. The numbers and strings at the end of the R file are 
arguments that are passed to the R script. 

GBM_reg_*.R (* represents the optimization algorithm): these did 10 runs for 
...
GBM_reg_opt_graphs: reads in all of the datasets from the optimization tests
