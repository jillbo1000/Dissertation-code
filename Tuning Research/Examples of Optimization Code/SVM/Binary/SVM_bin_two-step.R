...
svm.opt1 <- function(cost){
  pr <- NULL
  try(pr <- svm(as.factor(y) ~ ., data = dat, cost = cost, gamma = 32, cross = cross))
  if(!is.null(pr)){
...
svm.opt2 <- function(gamma){
  pr <- NULL
  try(pr <- svm(as.factor(y) ~ ., data = dat, cost = cost, gamma = 2^gamma, cross = cross))
  if(!is.null(pr)){
    err <- 100 - pr$tot.accuracy
  } else {
    err <- 100
...
for(i in 1:10) {
  t1 <- Sys.time()
  opt.obj <- optimize(svm.opt1, interval = c(1, 1024))
  cost <- opt.obj$minimum
  opt.obj2 <- optimize(svm.opt2, interval = c(-10, 10))
  if(!is.null(opt.obj) & !is.null(opt.obj)) {2
    alldat$Cost[i] = opt.obj$minimum
    alldat$Gamma[i] = opt.obj2$minimum
    alldat$Error[i] = opt.obj2$objective * 0.01
  }  
  t2 <- Sys.time()
