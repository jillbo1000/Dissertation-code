...
for(i in 1:10) {
  t1 <- Sys.time()
  da.obj <- DA(FUN = svm.opt, optimType = "MIN", numVar = 5,  
               numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(da.obj)) {
    alldat$Cost[i] = da.obj[1]
    alldat$Gamma[i] = round(da.obj[2])
    alldat$Epsilon[i] = round(da.obj[3])
    alldat$MSE[i] = svm.opt(da.obj)
  }
