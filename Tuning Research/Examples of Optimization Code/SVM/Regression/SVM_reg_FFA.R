...
for(i in 1:10) {
  t1 <- Sys.time()
  ffa.obj <- FFA(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(ffa.obj)) {
    alldat$Cost[i] = ffa.obj[1]
    alldat$Gamma[i] = round(ffa.obj[2])
    alldat$Epsilon[i] = round(ffa.obj[3])
    alldat$MSE[i] = svm.opt(ffa.obj)
  }
