...
for(i in 1:10) {
  t1 <- Sys.time()
  goa.obj <- GOA(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(goa.obj)) {
    alldat$Cost[i] = goa.obj[1]
    alldat$Gamma[i] = round(goa.obj[2])
    alldat$Epsilon[i] = round(goa.obj[3])
    alldat$MSE[i] = svm.opt(goa.obj)
  }
