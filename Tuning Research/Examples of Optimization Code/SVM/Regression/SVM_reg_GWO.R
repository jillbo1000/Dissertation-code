...
for(i in 1:10) {
  t1 <- Sys.time()
  gwo.obj <- GWO(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(gwo.obj)) {
    alldat$Cost[i] = gwo.obj[1]
    alldat$Gamma[i] = round(gwo.obj[2])
    alldat$Epsilon[i] = round(gwo.obj[3])
    alldat$MSE[i] = svm.opt(gwo.obj)
  }
