...
for(i in 1:10) {
  t1 <- Sys.time()
  hs.obj <- HS(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(hs.obj)) {
    alldat$Cost[i] = hs.obj[1]
    alldat$Gamma[i] = round(hs.obj[2])
    alldat$Epsilon[i] = round(hs.obj[3])
    alldat$MSE[i] = svm.opt(hs.obj)
  }
