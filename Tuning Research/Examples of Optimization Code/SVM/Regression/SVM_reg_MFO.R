...
for(i in 1:10) {
  t1 <- Sys.time()
  mfo.obj <- MFO(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(mfo.obj)) {
    alldat$Cost[i] = mfo.obj[1]
    alldat$Gamma[i] = round(mfo.obj[2])
    alldat$Epsilon[i] = round(mfo.obj[3])
    alldat$MSE[i] = svm.opt(mfo.obj)
  }
