...
for(i in 1:10) {
  t1 <- Sys.time()
  pso.obj <- PSO(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(pso.obj)) {
    alldat$Cost[i] = pso.obj[1]
    alldat$Gamma[i] = round(pso.obj[2])
    alldat$Epsilon[i] = round(pso.obj[3])
    alldat$MSE[i] = svm.opt(pso.obj)
  }
