...
for(i in 1:10) {
  t1 <- Sys.time()
  sca.obj <- SCA(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(sca.obj)) {
    alldat$Cost[i] = sca.obj[1]
    alldat$Gamma[i] = round(sca.obj[2])
    alldat$Epsilon[i] = round(sca.obj[3])
    alldat$MSE[i] = svm.opt(sca.obj)
  }
