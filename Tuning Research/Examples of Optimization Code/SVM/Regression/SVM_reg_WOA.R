...
for(i in 1:10) {
  t1 <- Sys.time()
  woa.obj <- WOA(FUN = svm.opt, optimType = "MIN", numVar = 5,  
                 numPopulation = 10, maxIter = 10, rangeVar)
  t2 <- Sys.time()
  if(!is.null(woa.obj)) {
    alldat$Cost[i] = woa.obj[1]
    alldat$Gamma[i] = round(woa.obj[2])
    alldat$Epsilon[i] = round(woa.obj[3])
    alldat$MSE[i] = svm.opt(woa.obj)
  }
