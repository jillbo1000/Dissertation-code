...
for(i in 1:10) {
  t1 <- Sys.time()
  nmkb.obj <- nmkb(par = c(cstart, gstart, estart), fn = svm.opt, 
                   lower = c(cmin, gmin, emin), upper = c(cmax, gmax, emax))
  t2 <- Sys.time()
  if(!is.null(nmkb.obj)) {
    alldat$Cost[i] = nmkb.obj$par[1]
    alldat$Gamma[i] = nmkb.obj$par[2]
    alldat$Epsilon[i] = nmkb.obj$par[3]
    alldat$MSE[i] = nmkb.obj$value
  }
