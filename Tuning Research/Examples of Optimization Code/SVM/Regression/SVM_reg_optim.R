...
for(i in 1:10) {
  t1 <- Sys.time()
  optim.obj <- optim(par = c(cstart, gstart, estart), fn = svm.opt, 
                     lower = c(cmin, gmin, emin), upper = c(cmax, gmax, emax), 
                     method = "L-BFGS-B")
  t2 <- Sys.time()
  if(!is.null(optim.obj)) {
    alldat$Cost[i] = optim.obj$par[1]
    alldat$Gamma[i] = optim.obj$par[2]
    alldat$Epsilon[i] = optim.obj$par[3]
    alldat$MSE[i] = optim.obj$value
  }
